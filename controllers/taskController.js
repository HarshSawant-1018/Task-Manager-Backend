...
const updateTask = async (req, res) => {
    ...
    if(req.body.assignedTo) {
        if(!Array.isArray(req.body.assignedTo)){
            return res
                .status(400)
                .json({message: "assginedTo must be an array of user IDs"});
        }
...
const updateTaskStatus = async(req, res) => {
    ...
    if(!isAssigned && req.user.role !== "admin"){
        return res.status.json(403).json({ message: "Not Authorized"});
    }
...
const getDashboardData = async (req, res) => {
    try {
        ...
        console.log(taskDistributedRaw);
        ...
        taskDistributedRaw["All"] = totalTasks; // Add total count to taskDistribution

        // Ensure all partially levels are included
